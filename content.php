<?php
	...
	$name = "SELECT * FROM dc_newsinfo WHERE catid = 37";
	...
	while($row = mysql_fetch_object($result)){
		...
		$naem = mysql_query("INSERT INTO `fm_post_metas`(`post_id`,`key`,`value`) VALUES ($name3[0],'campus','龙洞')");
	}

	$name = "SELECT * FROM dc_newsinfo WHERE catid = 38";
	$result = mysql_query($name);
	while($row = mysql_fetch_object($result)){
			$sam = mysql_query("INSERT INTO fm_posts (title,content,source,author_id,created_date,status)
			VALUES('$row->title','$row->newscontent','$row->source','1','$row->addtime','1')");
	...
	$name = "SELECT * FROM dc_newsinfo WHERE catid = 40";
	$result = mysql_query($name);
	while($row = mysql_fetch_object($result)){
			$sam = mysql_query("INSERT INTO fm_posts (title,content,source,author_id,created_date,status)
			VALUES('$row->title','$row->newscontent','$row->source','1','$row->addtime','1')");
	...
	$name = "SELECT * FROM dc_newsinfo WHERE catid = 41";
	$result = mysql_query($name);
	while($row = mysql_fetch_object($result)){
			$sam = mysql_query("INSERT INTO fm_posts (title,content,source,author_id,created_date,status)
			VALUES('$row->title','$row->newscontent','$row->source','1','$row->addtime','1')");
