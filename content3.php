<?php
	...
	$name = "SELECT * FROM dc_newsinfo WHERE catid = 42";
	...
	while($row = mysql_fetch_object($result)){
		...
		$naem = mysql_query("INSERT INTO `fm_post_metas`(`post_id`,`key`,`value`) VALUES ($name3[0],'campus','东风路')");
	}

	$name = "SELECT * FROM dc_newsinfo WHERE catid = 43";
	$result = mysql_query($name);
	while($row = mysql_fetch_object($result)){
			$sam = mysql_query("INSERT INTO fm_posts (title,content,source,author_id,created_date,status)
			VALUES('$row->title','$row->newscontent','$row->source','1','$row->addtime','1')");
	...
	$name = "SELECT * FROM dc_newsinfo WHERE catid = 44";
	$result = mysql_query($name);
	while($row = mysql_fetch_object($result)){
			$sam = mysql_query("INSERT INTO fm_posts (title,content,source,author_id,created_date,status)
			VALUES('$row->title','$row->newscontent','$row->source','1','$row->addtime','1')");
